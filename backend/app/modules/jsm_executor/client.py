...
class JsmExecutor:
    ...
    def post_comment(self, issue_key: str, body: str, public: bool = True) -> dict:
        ...
        with httpx.Client() as client:
            response = client.post(url, json=payload, headers=self._get_auth_header())
    ...
    def get_transitions(self, issue_key: str) -> dict:
        url = f"{settings.jsm_base_url}/rest/api/3/issue/{issue_key}/transitions"
        with httpx.Client() as client:
            response = client.get(url, headers=self._get_auth_header())
            response.raise_for_status()
            return response.json()
    ...
    def assign_issue(self, issue_key: str, account_id: str) -> bool:
        ...
        with httpx.Client() as client:
            response = client.put(url, json=payload, headers=self._get_auth_header())
            return response.status_code == 204